...
During class on Tuesday, I'd like to discuss some of my chi-test results with data points less than 5, how to set up bulleted lists, and how to add a % sign to a y-axis.
...
```{r}
anova <- aov (pct_no_internet ~ county, data = tract_data)
summary(anova)
anova <- aov (med_incomeE ~ county, data = tract_data)
summary(anova)
anova <- aov (avg_hh_sizeE ~ county, data = tract_data)
summary(anova)
```
...
```{r}
anova <- aov (pct_no_internet ~ maj_re, data = tract_data)
summary(anova)
anova <- aov (med_incomeE ~ maj_re, data = tract_data)
summary(anova)
anova <- aov (avg_hh_sizeE ~ maj_re, data = tract_data)
summary(anova)
```
...
```{r}
ggplot(tract_data, aes(x = county, y = pct_no_internet*100)) +
  geom_boxplot() +
  theme_bw() +
